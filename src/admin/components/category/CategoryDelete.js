...
function CategoryDelete({ isCategoryDelete, setCategoryDelete }) {
    return (
        <div>
            <div
                className={`fixed inset-0 bg-gray-900 opacity-50 transition-opacity ${isCategoryDelete ? 'block' : 'hidden'}`}
                onClick={() => setCategoryDelete(false)}></div>
            <div className={`fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform ${isCategoryDelete ? 'translate-x-0' : 'translate-x-full'
                } bg-white dark:bg-gray-800`}
                tabIndex="-1"
                aria-hidden={isCategoryDelete ? 'false' : 'true'}
            >
                ...
                <h3 className="mb-6 text-lg text-gray-500 dark:text-gray-400">Bạn có chắc chắn muốn xóa danh mục này không?</h3>
                <button
