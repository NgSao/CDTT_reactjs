...
function BannerDelete({ isBannerDelete, setBannerDelete }) {
    return (
        <div>
            <div
                className={`fixed inset-0 bg-gray-900 opacity-50 transition-opacity ${isBannerDelete ? 'block' : 'hidden'}`}
                onClick={() => setBannerDelete(false)}></div>
            <div className={`fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform ${isBannerDelete ? 'translate-x-0' : 'translate-x-full'
                } bg-white dark:bg-gray-800`}
                tabIndex="-1"
                aria-hidden={isBannerDelete ? 'false' : 'true'}
            >
