...
function PostEdit({ isPostEdit, setPostEdit }) {
    return (
        <>
            <div className={`fixed inset-0 bg-gray-900 opacity-80 ${isPostEdit ? 'block' : 'hidden'} transition-opacity`} aria-hidden="true"></div>

            <div className={`fixed inset-0 z-50 flex items-center justify-center overflow-x-hidden overflow-y-auto ${isPostEdit ? 'block' : 'hidden'}`} aria-hidden="true">
