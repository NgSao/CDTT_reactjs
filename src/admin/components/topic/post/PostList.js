...
function PostList() {
    ...
    return (
        <main>
                                    ...
                                    {posts && posts.length > 0 && posts.map((post, index) => {
                                        return (
                                            <tr key={index} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                                ...
                                                <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">#{post.id}
                                                </td>

                                                <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400 flex flex-row items-start">
                                                    <img
                                                        src={`${ImageApi}/images/post/${post.thumbnail}`}
                                                        alt={post.thumbnail}
                                                        className="w-16 h-16 object-cover"
                                                    />
                                                    <div className='flex flex-col ml-2'>
                                                        <div className="text-base font-semibold text-gray-900 dark:text-white">{post.title}</div>
                                                        <div className="text-sm font-normal text-gray-500 dark:text-gray-400">{post.slug}</div>
                                                    </div>
                                                </td>
                                                <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                    {post.topic_name ? post.topic_name : "Chưa có chủ đề"}
                                                </td>

                                                <td className="p-4 text-base font-medium  whitespace-nowrap text-gray-500 dark:text-gray-400">{post.content}</td>

                                                <td className="p-4 text-base font-medium  whitespace-nowrap text-gray-500 dark:text-gray-400">{post.description}</td>
                                                ...
                                                <td className="p-4 text-base font-normal text-gray-900 whitespace-nowrap dark:text-white">
                                                    <span
                                                        className={`text-xs font-medium mr-2 px-2.5 py-0.5 rounded-md ${post.status === 1 ? 'bg-green-100 text-green-800 border border-green-100 dark:bg-gray-700 dark:text-green-400 dark:border-green-500' : 'bg-yellow-100 text-yellow-800 border border-yellow-100 dark:bg-gray-700 dark:text-yellow-400 dark:border-yellow-500'}`} >
                                                        {post.status === 1 ? 'Hoạt động' : 'Tạm ngừng'}
                                                    </span>
                                                </td>
            ...
            <PostAdd isPostAdd={isAdd} setPostAdd={setAdd} />
            <PostEdit isPostEdit={isEdit} setPostEdit={setEdit} />
            <PostDelete isPostDelete={isDelete} setPostDelete={setDelete} />
