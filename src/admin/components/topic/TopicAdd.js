...
function TopicAdd({ isTopicAdd, setTopicAdd }) {
    return (
        <>
            <div className={`fixed inset-0 bg-gray-900 opacity-80 ${isTopicAdd ? 'block' : 'hidden'} transition-opacity`} aria-hidden="true"></div>
            <div className={`fixed inset-0 z-50 flex items-center justify-center overflow-x-hidden overflow-y-auto ${isTopicAdd ? 'block' : 'hidden'}`} aria-hidden="true">
                <div className="relative w-full h-full max-w-2xl px-4 md:h-auto">
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
                        <div className="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700">
                            ...
                            <button onClick={() => setTopicAdd(false)}
