...
function TopicList() {
    ...
    return (
        <main>
                                    ...
                                    {topics && topics.length > 0 && topics.map((topic, index) => {
                                        return (
                                            <tr key={index} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                                ...
                                                <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">#{topic.id}
                                                </td>

                                                <td className="flex items-center p-4 mr-12 space-x-6 whitespace-nowrap">
                                                    <div className="text-sm font-normal text-gray-500 dark:text-gray-400">
                                                        <div className="text-base font-semibold text-gray-900 dark:text-white">{topic.name}</div>
                                                        <div className="text-sm font-normal text-gray-500 dark:text-gray-400">{topic.slug}</div>
                                                    </div>
                                                </td>

                                                <td className="p-4 text-base font-medium  whitespace-nowrap text-gray-500 dark:text-gray-400">{topic.description}</td>
                                                ...
                                                <td className="p-4 text-base font-normal text-gray-900 whitespace-nowrap dark:text-white">
                                                    <span
                                                        className={`text-xs font-medium mr-2 px-2.5 py-0.5 rounded-md ${topic.status === 1 ? 'bg-green-100 text-green-800 border border-green-100 dark:bg-gray-700 dark:text-green-400 dark:border-green-500' : 'bg-yellow-100 text-yellow-800 border border-yellow-100 dark:bg-gray-700 dark:text-yellow-400 dark:border-yellow-500'}`} >
                                                        {topic.status === 1 ? 'Hoạt động' : 'Tạm ngừng'}
                                                    </span>
                                                </td>
            ...
            <TopicAdd isTopicAdd={isAdd} setTopicAdd={setAdd} />
            <TopicEdit isTopicEdit={isEdit} setTopicEdit={setEdit} />
            <TopicDelete isTopicDelete={isDelete} setTopicDelete={setDelete} />
